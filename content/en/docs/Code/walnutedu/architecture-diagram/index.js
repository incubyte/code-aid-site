...
const couplingFilterMin = document.getElementById("coupling-threshold");
const couplingFilterValue = document.getElementById("coupling-threshold-value");

const couplingCheckbox = document.getElementById("couplings-checkbox");
const codeDependencyCheckbox = document.getElementById("code-dependencies-checkbox");
...
resetFiltersButton.addEventListener("click", () => {
    ...
    updateCouplingFilterValue();
    applyFilters();
});

const data = fetch("./d3-diagram-tel.json")
    .then((res) => res.json())
...
const chart = () => {
    ...
    const node = svg
        ...
        .on("click", function (e, d) {
            ...
            linkPaths
                .filter((l) => l.type === "coupling")
                .filter((l) => isInsideOrEqualNode(d, l.source))
                .style("stroke", (d) => "#0eb601")
    ...
    const links = root
        ...
        .flatMap((d) => {
                const allLinks = [];
                if (codeDependencyCheckbox.checked) {
                    allLinks.push(d.data.dependencies?.map((dep) => ({
                        source: d,
                        target: findNodeByName(root, dep.to),
                        type: dep.type,
                    })));
                }
                if (couplingCheckbox.checked) {
                    allLinks.push(d.data.changecouplings
                        ?.filter((dep) => dep.strength >= couplingFilterMin.value)
                        .map((dep) => ({
                            source: d,
                            target: findNodeByName(root, dep.to),
                            type: dep.type,
...
function updateCouplingFilterValue() {
    couplingFilterValue.textContent = couplingFilterMin.value;
}
...
couplingFilterMin.addEventListener("input", function () {
    updateCouplingFilterValue();
});
...
function dependencyAnalysis(data) {
    ...
    while (nodesToVisit.size > 0) {
        ...
        if (node.dependencies && codeDependencyCheckbox.checked) {
            allDependencies.push(...node.dependencies);
        }
        if (node.changecouplings && couplingCheckbox.checked) {
            allDependencies.push(node.changecouplings);
        }
        for (const dependency of allDependencies) {
            ...
            nodesToVisit.get(dependencyNode)[optimalModule] += dependency.type === "coupling" ? dependency.strength : 1;
        }
...
function findAllNodes(data, shouldIncludeFn) {
    ...
    while (stack.length > 0) {
        ...
        // if (!node.name) {console.log(node); continue;}
