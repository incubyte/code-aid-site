...
const mapToArray = (map) => Array.from(map, ([key, value]) => ({ key, value }));
const getSecurityIssuesMetadata = (results) => {
    const allLanguagesWithCount = new Map();
    const allSeveritiesWithCount = new Map();
    results.forEach((result) => {
        var _a;
        const language = result.language || "Unknown";
        const severity = ((_a = result.vulnerabilities) === null || _a === void 0 ? void 0 : _a.severity) || "Unknown";
        allLanguagesWithCount.set(language, (allLanguagesWithCount.get(language) || 0) + 1);
        allSeveritiesWithCount.set(severity, (allSeveritiesWithCount.get(severity) || 0) + 1);
    });
    return {
...
function splitByVulnerablity(issue) {
